...
def render_document_extractor():
    ...
    def render_agent2_workspace():
        ...
        if selected_record:
            ...
            with m_col3:
                val_due_date = st.text_input("Payment Maturity Due Date", value=str(doc_header.get("dueDate") or ""), key="doc_due_val")
            with m_col4:
            ...
            with c_col3:
                val_customer = st.text_input("Customer Entity Name", value=str(customer_header.get("name") or ""), key="doc_cust_val")
            with c_col4:
                val_customer_gst = st.text_input("Customer Corporate GSTIN", value=str(customer_header.get("gstin") or ""), key="doc_cgst_val")
            ...
            with f_col3:
                ...
                val_discount = st.number_input("Absolute Discount Deductions", value=disc_val, key="doc_disc_val")
            with f_col4:
            ...
            val_round = st.number_input("Round Off Adjustment", value=round_val, key="doc_rnd_val", label_visibility="collapsed")
            ...
            edited_line_items = st.data_editor(
                ...
                width="stretch", 
                num_rows="dynamic",
                key=f"data_grid_editor_{selected_record.get('doc_id')}"
            )
            ...
            with act_col1:
                if st.button("📥 Approve & Commit Document Draft parameters to ERP Ledger", type="primary", width="stretch"):
                    ...
                    compiled_human_data["document"] = {
                        "documentNo": val_doc_no,
                        "documentDate": val_doc_date,
                        "dueDate": val_due_date,
                        "paymentTerms": val_terms,
                        "subtotal": val_subtotal,
                        "taxAmount": val_tax,
                        "discount": val_discount,
                        "totalAmount": val_total,
                        "roundOff": val_round
                    }
    ...
    processed_items = render_agent2_workspace()

    if processed_items:
        st.markdown("---")
        st.subheader("📜 Historical Document Processing Log")
        st.markdown("Review document attachments that have already passed through manual review validation rules.")
